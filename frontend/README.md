...
- Verify saved inventory reloads existing saved APIs from Agents/Sync or the selected Explorer snapshot. It starts no provider collection or clearing, suppresses automatic identity backfill for that verification read (including page correction), and preserves revision/session fences. Pending and failed checks replace the prior green receipt explicitly. Saved verification time (`checkedAt`) is separate from source collection time (`observedAt`); rechecking never renews native-control freshness.
...
- Invalid matching metadata is visible on Agents, Sync and the affected agent's detail, with explicit selected-package or full-inventory refresh instructions. Checked package counts are not valid-metadata or matched-agent counts, and invalid counts alone never trigger automatic retry. Source-specific identity warnings remain in Technical details without changing the backend's link state.
