...
describe("SyncHistoryTable", () => {
  it("combines sync runs and refreshes without tabs, preserving units and detail links but excluding unrelated jobs", async () => {
    ...
    expect(within(table).getAllByRole("row")[1]).toHaveTextContent("Power Platform refresh");
    expect(within(table).getAllByRole("row")[2]).toHaveTextContent("Old Graph refresh");
    expect(within(table).getAllByRole("row")[3]).toHaveTextContent("Sync 1");
    expect(within(table).getByText("Users")).toBeVisible();
    expect(screen.getByText("Sync 1").closest("tr")).toHaveTextContent("1 of 1 sources complete");
    expect(screen.getByText("Old Graph refresh").closest("tr")).toHaveTextContent("40 records saved");
    expect(screen.getByText("Power Platform refresh").closest("tr")).toHaveTextContent("0 records saved");
  ...
  it("sorts and paginates all three sources together and resets the page on outcome changes", async () => {
    render(<SyncHistoryTable state={projection(Array.from({ length: 13 }, (_, index) => job(index, {
      source: index % 3 === 0 ? "data-sync" : index % 3 === 1 ? "package-refresh" : "power-platform",
      status: index % 3 === 0 ? "completed" : "succeeded",
  ...
  it.each(["data-sync", "package-refresh", "power-platform"] as const)("does not present unavailable %s history as successfully empty", source => {
    render(<SyncHistoryTable state={{
      ...projection([]), unavailableSources: [{ source, code: "source_unavailable" }],
    }} error="" onRefresh={vi.fn()} />);
  ...
  it("includes retained runs and both Graph authorization modes in the bounded combined count", () => {
    ...
    expect(screen.getByText(/1-10 of 80 recent records/)).toHaveTextContent(
      "Recent history only; filters apply to the loaded records.",
    );
  });

  it("uses each row's recorded start without substituting a source job's creation time", () => {
    render(<SyncHistoryTable state={projection([
      job(1),
      job(2, {
    ...
    expect(screen.getByRole("columnheader", { name: "Duration" })).toHaveAttribute(
      "title", "Time from the recorded start to completion",
    );
