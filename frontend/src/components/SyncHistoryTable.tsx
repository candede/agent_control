...
import { formatSyncInstant, syncDuration, syncSourceDetails, syncStatusLabel } from "./syncPresentation";
import "./dataSync.css";
...
export function SyncHistoryTable({ state, error, onRefresh, onOpenSyncRun }: {
  ...
  const rows = jobs.slice(currentPage * pageSize, (currentPage + 1) * pageSize);

  return (
    <section className="jobs-view sync-history" aria-labelledby="jobs-heading">
          ...
          <table className="sync-history-table" aria-label={view === "runs" ? "Sync run history" : "Source job history"}>
            <thead>
              <tr>
                ...
                <th scope="col">{view === "runs" ? "Scope" : "Source job"}</th>
                <th scope="col">Outcome</th>
                <th scope="col">Result</th>
                <th scope="col" title="Time since the original start, including waits and retries">Duration</th>
            ...
            <tbody>
              {rows.map(job => (
                <tr key={`${job.source}:${job.id}`}>
                  <td><time dateTime={job.startedAt ?? job.updatedAt}>{formatSyncInstant(job.startedAt ?? job.updatedAt)}</time>
                    {!job.startedAt ? <small>Last update; start not recorded</small> : null}</td>
                  ...
                  <td><span className={`status-badge status-${job.status.replaceAll("_", "-")}`}>{syncStatusLabel(job.status)}</span>
                    {job.partial && job.status !== "partial" ? <small>Partial results</small> : null}</td>
                  <td>{resultLabel(job)}</td>
                  <td>{job.startedAt && job.completedAt
                    ? syncDuration(job.startedAt, job.completedAt)
                  ...
                  <td><a href={job.source === "data-sync" ? `/sync?syncRun=${encodeURIComponent(job.id)}` : job.href}
                    aria-label={`View details for ${job.label} from ${formatSyncInstant(job.startedAt ?? job.updatedAt)}`}
                    onClick={event => {
      ...
      <p className="jobs-note">{state ? <>Last checked <time dateTime={state.polledAt}>{formatSyncInstant(state.polledAt)}</time>. </> : null}
...
function matchesOutcome(job: WorkbenchJobSummary, outcome: string) {
  const complete = job.status === "completed" || job.status === "succeeded";
...
function resultLabel(job: WorkbenchJobSummary) {
  if (job.source === "data-sync") {
    return job.completed !== null && job.total !== null ? `${job.completed} of ${job.total} sources complete` : "Count not reported";
  }
  if (job.status === "succeeded" && job.total !== null) return `${job.total.toLocaleString()} records saved`;
  return job.completed !== null ? `${job.completed.toLocaleString()} reported so far` : "Count not reported";
}
