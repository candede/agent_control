...
function importedUsage(username: string, displayName: string, count: number): OfficialUsageUserSummary {
  return {
    ...
    reviewCandidate: count <= 5, licenseAssignmentStatus: "unavailable", creatorTypes: ["Microsoft"],
    rows: count ? [{
    ...
    searchableText: `${username} researcher microsoft`,
...
export function licensedUser(index: number, displayName: string, count: number | null): CopilotUsageUser {
  ...
  return {
    ...
    licenses: [{
      ...
      state: "enabled", disabledPlanIds: [], assignmentStates: [{ state: "Active", error: null, assignedByGroup: null }],
    }],
    servicePlans: [{
...
export const copilotUsageFixture: CopilotUsageUsersResponse = {
  ...
  counts: { licensedUsers: 4, measuredActivityUsers: 3, needsAttentionUsers: 2, unknownMetricsUsers: 1, unresolvedImportedIdentities: 1 },
