...
**Legacy mode:** complete `tenantDomains` in the target as a nonempty array, for example `["contoso.com","contoso.onmicrosoft.com"]`. Optional `tenantDisplayName` is non-secret. These become `TENANT_DOMAINS` and optional `TENANT_DISPLAY_NAME` alongside the existing native credential references. Named-parameter equivalents are `-TenantDomains` and `-TenantDisplayName`. Missing domains stop setup rather than admitting arbitrary usernames.
...
2. Set `tenantRegistrySecretName` to `agent-control-tenants-json`. Add that name to `preparedVaultContract.secretNames` and `runtimeConsumers`, and its pinned non-secret version to `versions`. Keep the six base secrets; the registry must contain the approved primary tenant/application. `tenantDomains` may be empty because the registry is authoritative, not merged with legacy configuration.
...
4. On first migration, keep the six existing `existingVersions` entries unchanged and preserve the primary profile's original client secret inside the registry. On subsequent deployments include the registry's current version as the seventh `existingVersions` entry. Changes to any existing selected version require the coordinated-rotation prerequisite, not a version-check bypass.
