...
Closing the dialog does not accept or discard reports. Reopening it in the same application session preserves selected files and validation results. After reloading, reopen **Import reports** to restore server-side staging; files not yet staged must be selected again. Changing account, losing import access, or opening a different staging deep link unmounts the old importer and stops its follow-up uploads, reads, and refresh callbacks. An already submitted mutation may have completed server-side, so inspect retained staging before retrying. Authorization failures clear retained import details and approval actions. Exact staging links open the dialog automatically. Retained-set selection and deletion remain inside this management experience, with a separate confirmation.
...
**Official usage** owns tenant-wide agent reporting, not user profiles or license follow-up. Its **Agent activity** view starts with the selected snapshot, three headline measures (Agents-export responses, distinct positive-response report identities, and reported agents), and one **Agent comparison** table. Search and creator filters, plus most responses, most active users, fewest responses, latest activity, and name ordering, apply to the complete dataset before its 25-row pages. Unknown reach sorts after known counts, including explicit zero. The table replaces duplicate ranking tables, charts, and the separate active-window dashboard; catalog state remains an inventory concern, not an official usage measure.
