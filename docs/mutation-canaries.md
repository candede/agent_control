...
Access cycles use the same action for both directions. Creating their inverse approval preserves the current exact reverse transition under the same revisions; a replacement approval still expires the prior same-direction intent. Migration 38 makes verified access evidence unique per cycle stage so both directions can publish atomically, while retaining the existing block/unblock uniqueness boundary. This runtime requires the pending migration through the normal operator deployment workflow; no migration is performed by a canary.
...
Provider timestamps must contain a valid UTC calendar date and time, without day rollover. Impossible dates or times fail with `provider_schema` rather than becoming status evidence. Valid timestamps retain their exact original text, including up to seven fractional-second digits, for prestate and restoration comparisons.
