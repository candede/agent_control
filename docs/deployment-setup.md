...
### Unknown Power Platform coverage

A successful refresh can still produce **Partial unified inventory**. The refresh enumerates all returned pages; the separate coverage calculation uses the supported inventory role-template IDs from the authenticated ID token's optional `wids` claim. If none are present, `roleScope` and type coverage stay `unknown`, even when resource counts match and provider access checks succeed. `AgentControl.Admin` is an application role, not proof of an Entra inventory role. Unknown evidence does not prove the user lacks a provider role.

To resolve missing role evidence:

1. Have an authorized Entra administrator verify the user's active, tenant-wide supported inventory role. Activate an already approved PIM role if applicable; do not grant broader roles merely to clear a warning. Global Reader provides read-only full inventory coverage; AI Reader provides AI-scoped coverage including Copilot Studio agents.
2. Verify that the approved Agent Control **app registration** emits directory-role claims. Microsoft's [application manifest reference](https://learn.microsoft.com/en-us/entra/identity-platform/reference-app-manifest#groupmembershipclaims-attribute) supports `"groupMembershipClaims": "DirectoryRole"`; the [claims reference](https://learn.microsoft.com/en-us/entra/identity-platform/access-token-claims-reference) documents `wids` and the `DirectoryRole`/`All` configuration. For an app without existing group-claim requirements, `DirectoryRole` avoids requesting all group memberships. Preserve any existing required group claims and all unrelated registration settings; do not replace the registration or blindly overwrite a broader existing setting.
3. Sign out and sign in again so the application receives current claims. Session inventory-role metadata contains only recognized role-template IDs; do not copy or log raw tokens. Missing claims, an inactive/unsupported role, or malformed role evidence remain unknown; never fill in role IDs or coverage flags in the database.
4. In **Sync**, refresh Power Platform inventory without an environment restriction. Confirm the **new snapshot** shows the expected provider role scope and covered Copilot Studio type. Changing claims or passing a permission check does not retroactively change an old snapshot's coverage.

The repository's role manifest and deployment scripts do not apply this directory configuration or assign provider roles. With AI-scoped roles, excluded non-AI types remain `not_authorized_scope`, but Copilot Studio agent coverage can be covered. A single-environment snapshot remains partial for unified inventory; if role evidence is also unknown, both limitations are reported. Resource progress describes the saved request's scope, which may contain only agents, not all resource types. Even covered inventory remains subject to Microsoft's documented API exclusions, including classic/V1 bots; it is not proof of every possible agent surface.
