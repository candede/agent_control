...
Use **Verify saved inventory** to recheck the complete saved source set and reconciliation without provider calls. The unified API and CSV expose the check time, scope, source/unique/logical counts and identity checks. Scope limitations and matching problems remain explicit. Missing optional `wids` alone is not a partial-completion condition and does not require an app-registration change or broader role. Verification time does not advance source freshness or prove access beyond the recorded authenticated query.
