...
- **Duplicate-looking agents:** full agent sync now collects exact package identities automatically (four concurrent reads; 15-minute execution bound). Progress and failures are retained in Sync/Jobs, and publication is all-or-nothing. Existing saved summaries are backfilled once per snapshot when Agents is opened with current delegated read authorization; an already-running broad refresh is followed, and failed/cancelled attempts require an explicit Sync retry. The operation survives tab navigation but never changes agent settings. A process restart still requires real sign-in to reacquire delegated tokens. Identical display names are not identity proof; only corroborated environment/native metadata combines rows. Source diagnostics remain available under technical details.
