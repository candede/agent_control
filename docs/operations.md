...
- **Slow Graph package sync:** "Matching agent records" counts exact detail checks after catalog enumeration, not packages listed. The current collector unconditionally performs one detail read per listed package; this is an implementation choice, not proof that the list response lacks those details. Four continuously replenished workers avoid waiting for the slowest member of a fixed batch; healthy reads have no artificial pacing. Use the [package sync diagnostics](#graph-package-sync-diagnostics) to compare actual list/detail responses and waiting times before attributing the difference to paging, necessary enrichment, or throttling.
