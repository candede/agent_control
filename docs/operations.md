...
Apply all pending migrations through **36** with the migration operator and verify readiness with the restricted runtime role before deploying this sync workflow. Migration 36 changes only the clean-full admission constraint: all three automatic source IDs are required, and retained legacy runs may additionally contain `usage_reports`. It does not alter cleanup targets, grant direct DELETE access, modify old run intent, or apply migrations from the in-app reset action.
