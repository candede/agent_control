...
- **Sync readiness failures:** a cached readiness timeout, transport error, or throttling result is a failed check, not evidence of missing permission. Sync retains that cause instead of showing **Permission required** or attributing internal child-job cleanup to the user. **Retry incomplete** explicitly rechecks failed Graph package and Power Platform readiness for the selected sources; fresh successful checks and throttling cooldowns are reused, and provider collection still requires current authorization. Permission Center can become **Available** when its automatic check succeeds after evidence expires, without any change to consent. This does not itself replay failed sync work. Use `data_sync_source_failed` with the run/source/job IDs to diagnose startup failures; a child marked `data_sync_cleanup` is internal cleanup, not a user cancellation.
...
- **Slow Graph package sync:** "Matching agent records" counts exact detail checks after catalog enumeration, not packages listed. Unlike paged directory/resource collection, this stage needs one request per package for identity metadata. Four continuously replenished workers avoid waiting for the slowest member of a fixed batch; healthy reads have no artificial pacing. For unexplained pauses, inspect the retry events below before comparing object counts between providers. A fast list-only client is not doing the same work as a complete identity sync.
- **Graph 424 / UnknownError throttling:** Graph can wrap a "too many requests" response in HTTP 424. `outcome: throttled` distinguishes this from other dependency failures. Throttling pauses queued inventory reads, not just the failed request, and enables conservative pacing of at least 250 milliseconds between subsequent requests across jobs for that inventory client's lifetime. Without `Retry-After`, cooldowns are 30, 60, then 120 seconds, with at most six attempts. Valid `Retry-After` values up to five minutes are honored in full, including on 503; a longer requested cooldown fails rather than retrying early. Sync displays the wait and identities already checked even when other workers finish during the wait. Do not repeatedly click retry or clear the inventory during a cooldown. The complete collection still has its 15-minute deadline, and writes never retry.
...
Package refresh startup preserves the first cancellation, sign-out, or shutdown reason even if a pending job lookup, authorization check, or token request later rejects. Cleanup persistence failures still propagate to the caller and an in-flight shutdown drain. If cancellation or the execution deadline interrupts application-scope authorization before publication, no subsequent capability check or publication is started.
