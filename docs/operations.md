...
An authenticated Admin may call `GET /api/diagnostics`; it returns only auth-configured, maintenance/provider-work flags, schema version and fixed pool/body/export limits. Do not add record counts, hosts, connection strings or provider bodies.
...
Saved inventory is checked automatically; administrators do not approve it after sync. For troubleshooting, open **Sync > Advanced results** to inspect the receipt or use **Verify saved inventory** to recheck the complete saved source set and reconciliation without provider calls. Expanding the disclosure starts no sync or verification action. Agents retains only a concise attention notice rather than the diagnostic panel, and Sync also keeps real problems visible when diagnostics are collapsed. The unified API and CSV expose the check time, scope, source/unique/logical counts and identity checks. Scope limitations and matching problems remain explicit. Missing optional `wids` alone is not a partial-completion condition and does not require an app-registration change or broader role. Verification time does not advance source freshness or prove access beyond the recorded authenticated query.

Data sync labels a successful source's count **Saved count**, and an unfinished or unsuccessful source's count **Reported count**. A reported provider total is not a count of published rows and can exceed the number fetched so far. **Last successful** records a successful Data sync publication, not the latest failed attempt or an older standalone inventory refresh.
