...
- **Audit** shows only local administrative actions taken through this app. Old `source=purview`, `job` and `user` query parameters no longer select provider content and are removed while retaining valid local filters. Jobs links lead to Users or Agents; select the entity there to investigate. Legacy broad/multi-user jobs are not presented as one user's history.
