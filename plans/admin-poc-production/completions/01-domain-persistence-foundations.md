...
| --- | --- | --- | --- | --- | --- |
| P01-LIVE-IDENTITY | Real Entra/provider behavior unavailable in this local fixture deployment | Sign-in visibly unconfigured, no fake credentials or remote writes; access UI retains its preexisting hidden state | Any attempt to claim live provider success from fixtures or perform an operation without actual authorization | Tenant administrator; Phase 02 auth owner; Phase 05 control owner | Supply existing approved tenant/client configuration through restricted files, register the exact callback, then perform authorized sign-in/read/write checks. Current ordinary Admin-confirmed writes require no prior canary; qualification exercises are optional and separately approved. No automatic grant expansion or new live-success claim. |
