...
| --- | --- | --- | --- | --- | --- |
| P09-LIVE-QUARANTINE | No live version-1 status/rollout, delegated eligibility or write-transition proof was obtained; official version examples conflict | No arbitrary target probe, app credentials or fallback; exact targeting and current delegated authorization remain required. Current policy makes canaries optional and non-gating; no new live proof is claimed | Actual version/schema/authorization/readback failure requires request-specific remediation; missing optional canary approval means that exercise is not run | Tenant Power Platform administrator plus Phase 13 qualification owner | Use an exact native target/current private inventory and current Admin/delegated authority for confirmed on-demand operations. For a separately approved optional canary, verify version-1 bounded GET, execute the tested original/restoration cycle, verify final prestate and retain only minimal safe evidence. A contract/version change requires reviewed fixtures/revision invalidation before another canary approval, not a standing ordinary-write gate. |
...
- Reuse the quarantine API/types/target picker/shared controls and source-independent observations, full 25-target confirmation, idempotent receipts and read-only reconciliation. No package/Defender identity, qualification or retained approval can authorize quarantine. Current ordinary Admin-confirmed writes use actual delegated authorization and operation prestate/readback, never prior canary evidence; optional canaries retain their own approval/restoration rules.
...
confirmations and GET-only reconciliation. Target-specific canary qualification
is optional and never gates ordinary Admin-confirmed writes.
