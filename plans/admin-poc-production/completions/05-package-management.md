...
Block/unblock availability requires both directional live canaries for the deployed contract/configuration/auth revision. Contract, permission, delegated-auth mode or configuration changes invalidate prior qualification. Access/reassign do not become available merely because generic qualification exists; their operation-specific safety failures must first be resolved by a changed provider contract and new fixtures/review.
...
| --- | --- | --- | --- | --- | --- |
| P05-LIVE-PACKAGE | Tenant package eligibility, read contract and block/unblock convergence are unavailable without authorized identity/license/approval | Saved data remains independent; provider actions fail closed; block capability remains `preview_disabled`; no current qualification record | Any claim of live readiness or write success based on fixtures, consent or HTTP acceptance | Tenant administrator and Phase 13 live qualification owner | Configure the approved existing single-tenant app through restricted inputs; sign in with the required app role/license; consent only the needed capability; run explicit list/detail probes; use a dedicated nonproduction package and separately approved actor/restorer to execute and restore both directional canaries; retain only minimal safe evidence. |
| P05-ACCESS-LOST-UPDATE | Beta package access update has no documented ETag/`If-Match` or equivalent protection for replacing a full access collection | Adapter remains fixture-tested; read-only assignments stay visible; all product access preview/dispatch routes fail with `package_operation_safety_unavailable` | Provider publishes and honors a conditional-write/revision contract, or an independently reviewed contract acceptably bounds external lost updates | Phase 09 contract review and Phase 13 qualification owner | Update exact provider fixtures and safety model, prove selected/unselected preservation plus external-race behavior, then run separately approved availability/installation canaries. Generic block qualification is insufficient. |
| P05-REASSIGN-READBACK | Reassign operation accepts `userId`, but package detail exposes no owner readback and operation headers expose no conditional write | Exact adapter fixture retained; product action visible but disabled with Microsoft documentation link; no dispatch route | Provider exposes authoritative owner readback and an acceptable concurrency contract | Phase 09 contract review and Phase 13 qualification owner | Capture the promoted/changed contract, add exact owner/concurrency/restoration fixtures and only then perform a separately approved reversible canary. |
