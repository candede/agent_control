...
4. Implement single and bounded bulk quarantine/unquarantine with `AgentControl.Admin`, current delegated provider authorization and explicit confirmation. Use on-demand management without prior canaries or extra mode/configuration. Freeze exact target lists. Explain that makers may still see/test a quarantined bot while other channels cannot use it; package blocking is a separate control.
5. Reuse Phase 05 intent/idempotency and optional qualification workflow. Serialize local writes per native target, pre-read current state, reject changed prestate and skip already-correct targets. A status timestamp is evidence, not provider atomicity; use conditional writes only if documented and disclose concurrent-overwrite risk otherwise. Optional qualification proves only its exact exercised contract and never gates ordinary operations.
...
10. Extend `docs/mutation-canaries.md` with optional canary scope/approval, prestate, touched fields, restoration and conflict handling. Test recovery commands before live canary execution and invalidate qualification evidence when contract/auth/configuration changes. Preserve actual approval/restoration rules for explicitly requested canaries without making them prerequisites for ordinary writes.
...
Missing provider eligibility is an actual operation/authorization limitation; report its exact evidence and remediation without inventing eligibility. Missing canary approval means that optional exercise is not run, never that ordinary Admin-confirmed writes are withheld. Never guess a bot/environment to satisfy a test.
...
- Direct quarantine state is source-provenanced; on-demand writes are delegated, exact, confirmed, durable, audited and verified without prior-canary requirements.
