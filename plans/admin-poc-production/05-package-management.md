...
- Implemented block, unblock and access update require delegated Microsoft Graph `CopilotPackages.ReadWrite.All`, `AgentControl.Admin`, exact targets and explicit confirmation. No prior probe/canary ritual or extra mode/configuration is required; current provider authorization and operation prestate/readback are enforced at execution. Owner reassignment has no implemented product workflow or documented owner readback.
...
2. Request delegated `CopilotPackages.Read.All` independently from delegated `CopilotPackages.ReadWrite.All`, following the README's current consent-onboarding contract. Consent groups request only the needed capability. Support separately enabled application read access for an explicitly requested, scope-approved refresh, never a recurring collector. Read-only inventory remains usable without write consent. Opening Agents with no broad saved snapshot starts one initial delegated read-only refresh per authorized session; saved data loads thereafter and explicit Refresh retries failures. A Permissions first-page check alone does not import inventory. Startup never schedules collection.
...
5. For access writes, resolve and display users/groups before confirmation, reject unresolved/deleted/duplicate principals, preserve the unselected access target, reject ambiguous provider scope, and verify both selected and preserved access collections. Implement the documented adapter and deterministic fixtures. Run authorized Admin-confirmed access writes on demand without prior canaries or extra configuration. Preserve exact endpoint evidence and report real provider failures without converting them into a standing qualification gate; do not redirect users to a different provider as though it controlled the same package access state.
6. Use conditional writes only when the exact endpoint documents and honors ETag/`If-Match`. Otherwise serialize local operations per native target, compare a hash of mutation-relevant prestate immediately before dispatch, and return conflict if changed. This cannot eliminate a race with an external administrator after the pre-read: disclose the concurrent-overwrite risk without withholding implemented access/block functionality. Local leases and hashes must never be described as provider atomicity. Preserve unselected fields and do not resend the full object blindly.
...
8. Retain the documented preview reassignment contract, `POST https://graph.microsoft.com/beta/copilot/admin/catalog/packages/{id}/reassign` with JSON `{ "userId": "<Entra user object ID>" }`, delegated `CopilotPackages.ReadWrite.All`, and no application permission. Owner reassignment has no implemented product workflow or documented owner readback; report that limitation rather than calling it an artificially disabled implemented control or claiming it works. Any future implementation needs exact user resolution, confirmation, one-shot dispatch and authoritative ownership readback.
...
13. Own `docs/mutation-canaries.md` and optional qualification records consumed by Phases 09/13: exact target/type, actor/approval/expiry, contract version, minimal prestate/poststate and restoration criteria. These exercises never gate ordinary writes. Use ordinary role-restricted database storage with managed protection and finite retention, not custom content encryption or public completion-file payloads. Restore only touched fields; compare semantic state, stop on intervening external changes and prove the recovery command with fixtures first. Contract/auth-mode/configuration changes invalidate qualification evidence, not ordinary operation availability; record reuse or an explicitly approved rerun for the deployed release.
...
Preview mutation or tenant-contract failure does not stop the campaign. Keep package reads available, report actual provider/authorization failures for the affected request, record evidence and remediation, and continue. Missing or expired optional canary evidence must not gate ordinary Admin-confirmed writes. Reconcile uncertain sent work before new work on its target. Never mark a write successful from HTTP acceptance alone.
...
Create `plans/admin-poc-production/completions/05-package-management.md` with exact endpoint/version evidence for every operation, test/live-probe results, actual provider limitations or unimplemented workflows, optional canary outcomes, and Phase 06 preconditions.
