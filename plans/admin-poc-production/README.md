...
**Consent onboarding supersession (2026-09-12):** Sign-in requests outstanding consent for every implemented delegated capability up front using MSAL's additional-consent scopes, including Admin package-write permissions. Token acquisition stays resource/capability-specific and application permissions are not bundled into onboarding. Entra decides whether interaction is required; recovery does not unconditionally force another consent prompt. These rules supersede initial-login requirements in historical prompts and completion records.
