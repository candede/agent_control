...
**Authorization supersession (2026-09-12):** The current application uses only `AgentControl.Viewer` and `AgentControl.Admin`, with Admin including all Viewer access. This two-role contract and the current [security model](../../docs/security-model.md) supersede the original four-role requirements in the phase prompts below. Completion records remain historical evidence, not the current authorization contract. Do not reintroduce the old role separation when executing or reviewing a phase.

**Readiness supersession (2026-09-12):** Safe delegated access checks run automatically for the current assigned signed-in user and are bounded, coalesced and expiry-aware. They do not trigger provider writes, mutation canaries, audit-query creation or hunting queries on page load. Token-only checks must be distinguished from successful provider calls; unchecked access is not a permission denial. Ordinary explicitly requested bounded delegated investigations establish access through their actual provider request rather than a separate qualification-button workflow. Disabled application modes are optional configuration, not failed delegated access. Application/shared-scope approvals and genuine unsupported-write restrictions remain separate. These rules supersede manual-probe and delegated read-qualification prerequisites in the historical phase prompts.

**Consent onboarding supersession (2026-09-12):** Normal sign-in requests outstanding delegated observational consent up front using MSAL's additional-consent scopes. An explicit deferred-setup path preserves identity-only login. Token acquisition stays resource/capability-specific; application permissions and Admin-only package-write scopes are not bundled into onboarding. Entra decides whether interaction is required; recovery does not unconditionally force another consent prompt. These rules supersede identity-only initial-login requirements in historical prompts and completion records.

**On-demand controls and inventory supersession (2026-09-12):** Implemented package access, block/unblock and quarantine management run on demand for an authorized, confirming Admin without prior canaries, qualification records or extra mode/configuration. Preserve provider tokens/permissions, app roles, exact targets, same-origin/CSRF, confirmation, audit, immediate prestate checks and readback. Optional canaries retain their actual approval/restoration requirements, never gate ordinary operations, and expired or absent canary evidence is not a reason to withhold implemented controls. Missing conditional-write protection is a concurrent-overwrite risk to disclose, not an enablement prerequisite. Owner reassignment has no implemented product workflow or documented owner readback; classic bots and actual Microsoft limits remain unsupported. Opening Agents with no saved broad snapshot starts one initial delegated read-only inventory refresh per authorized session; saved data loads thereafter and explicit Refresh retries failures. A Permissions first-page check alone is not an inventory import. These rules supersede contrary gate or navigation instructions in historical prompts/completions without changing their recorded test or live-evidence results.
...
- Inspect and operate implemented package controls and Copilot Studio quarantine on demand with exact targets, confirmation, audit and verification; qualification exercises are optional.
...
5. **On-demand work only.** Provider imports, searches and mutations start from explicit authorized actions. Opening Agents without a broad saved snapshot may start one initial delegated read-only refresh per authorized session; subsequent refresh/retry is explicit. Bounded in-process execution and polling may complete those durable jobs; restarting must not create recurring collection. Saved records show their last observation and scope. Ordinary expiry/retention cleanup may run without becoming a provider scheduler.
...
8. **Least privilege and preview visibility.** Request delegated observational consent up front with an explicit identity-only deferral; obtain tokens separately for each resource/capability and request Admin-only package-write permissions only for those controls. Application access is separately enabled for explicitly requested Graph reads/investigations with administrator-approved data scope, never unattended collectors. Implemented preview writes run on demand with current Admin/provider authority and confirmation; optional qualification evidence never gates them. Disclose preview and concurrent-overwrite risks.
...
| Auth, Viewer/Admin roles, data scope, capabilities, credentials, Entra manifest and Key Vault setup guide | 02                   | All routes/providers; UI 03; security 11; vault wizard/references 12; live 13               |
...
| Package controls           | Graph `CopilotPackages.ReadWrite.All`                                                                           | Delegated only; Agent Control Admin; Agent 365 license; preview/global-cloud constraints; explicit confirmation, immediate prestate checks and readback; no prior canary required.                                                                                   |
...
| Official CSV import        | No added Microsoft API permission                                                                               | Local Admin import; Viewer and Admin may read accepted reports.                                                                                                                                                                                                  |
...
The only internal roles are Viewer and Admin. Admin includes every Viewer permission; one assignment per approved user or group is sufficient. Set enterprise-application Assignment required to Yes. Neither role grants Microsoft provider permissions or bypasses delegated consent, service-side roles or operation safety checks.

| App role                      | Authorized data/actions                                                                                  |
| ----------------------------- | -------------------------------------------------------------------------------------------------------- |
| `AgentControl.Viewer`         | Inventory, exact-target observations, aggregate/user-level reports, audit, investigations and own read-job lifecycle |
| `AgentControl.Admin`          | All Viewer access plus supported provider changes, write qualification, imports and configuration         |

Import preview is limited to the Admin's submitted data and validation task; accepted reports are readable by Viewer and Admin. Export authorization equals source-read authorization. Admin access never implicitly grants another principal's private job results.

Delegated saved results are private to their initiating principal by default. Shared application-mode datasets require explicit Admin-approved scope and a current Viewer or Admin assignment; read-only application work is still manually requested, with no automatic app-only fallback for denied delegated access. Official report imports are explicitly tenant-shared; both roles may read aggregate and user-level projections. Scope every row/request to the configured tenant and reject mismatches. Filter before joins, counts, pagination, result retrieval and exports; a cross-source association never bypasses filtering. Recheck current role/data scope when accessing stored results, not just at submission. Provider unavailability does not erase authorized saved data.
