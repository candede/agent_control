...
4. Qualify delegated/application package reads as separately approved. Run optional package-write canaries only on approved reversible exact targets with Phase 05 prestate/restoration. Verify semantic touched-field state, preserve intervening changes and never retry ambiguous writes automatically. Absent or failed canary evidence does not gate ordinary Admin-confirmed access/block operations. Disclose the access endpoint's concurrent-overwrite risk; owner reassignment has no implemented product workflow or documented owner readback. Failed restoration is an owned incident requiring reconciliation of the affected target and verified recovery or explicit isolated-target retirement/containment acceptance, not a global qualification gate; never label it restored without proof.
...
9. Qualify direct quarantine status on an exact approved target. Only with explicit reversible canary approval perform the optional quarantine/unquarantine qualification cycle and verified restoration under Phase 05/09 policy. This approval/evidence is never required for ordinary Admin-confirmed writes. Classic bots are unsupported and missing IDs cannot target an operation; package state remains independent.
...
13. For reproducible defects add focused regression coverage, repair the owning code, run focused/applicable aggregate checks in Docker and redeploy through `deploy-azure.ps1` using the approved target/vault/budget/maintenance contract. Verify new artifact/revision and rerun affected checks; rerun optional canaries only with current explicit approval, never as a runtime prerequisite for ordinary writes. Do not patch production out of band, write vault values or silently rotate credentials.
14. Reconcile only retained capability issues. Each actual provider or authorization limitation has exact UX/remediation, safe evidence, containment and operator/recheck trigger. Missing optional qualification remains an evidence gap, not an app-imposed write gate. Future ideas are absent from the app/grants/resources and never count as unimplemented campaign work. Link origin issues rather than copying every prior table.
...
- Exact deployed core paths are healthy and observable; report real provider results or actual authorization/contract limitations with concrete remediation. Missing optional live qualification is not an ordinary-write gate.
