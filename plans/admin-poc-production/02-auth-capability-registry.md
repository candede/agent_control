...
3. Request outstanding consent for all implemented delegated capabilities during sign-in, including package changes and Copilot Studio quarantine, using MSAL additional-consent scopes. Acquire tokens separately for each resource/capability; application permissions remain separately configured. Add authenticated, CSRF-bound consent recovery routes that request one capability group and return safely to the application. Entra decides whether additional consent or sign-in interaction is required.
