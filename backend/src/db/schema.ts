...
import { copilotServiceSnapshotResetMigrationSql, dataSyncMigrationSql } from "./dataSyncSchema.js";
...
CREATE TABLE power_platform_refresh_jobs (
  ...
  requested_types jsonb NOT NULL CHECK (jsonb_typeof(requested_types) = 'array' AND jsonb_array_length(requested_types) BETWEEN 1 AND 2 AND requested_types <@ '["microsoft.copilotstudio/agents","microsoft.powerplatform/environments"]'::jsonb),
...
CREATE TABLE power_platform_inventory_snapshots (
  ...
  environment_scope text NOT NULL DEFAULT '' CHECK (length(environment_scope) <= 512),
  requested_types jsonb NOT NULL CHECK (jsonb_typeof(requested_types) = 'array' AND jsonb_array_length(requested_types) BETWEEN 1 AND 2 AND requested_types <@ '["microsoft.copilotstudio/agents","microsoft.powerplatform/environments"]'::jsonb),
  coverage jsonb NOT NULL CHECK (jsonb_typeof(coverage) = 'array' AND jsonb_array_length(coverage) = 2 AND octet_length(coverage::text) <= 8192),
...
CREATE TABLE power_platform_inventory_resources (
  ...
  resource_type text NOT NULL CHECK (resource_type IN ('microsoft.copilotstudio/agents','microsoft.powerplatform/environments')),
