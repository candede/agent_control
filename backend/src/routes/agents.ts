...
import { requirePackageMutationOperationSafe } from "../services/packageMutationSafety.js";
...
policyRoute(agentsRouter, "get", "/agents", { access: "authenticated", dataClass: "private_inventory", roles: ["AgentControl.Reader"] }, async (request, response) => {
...
policyRoute(agentsRouter, "get", "/directory/principals", { access: "authenticated", dataClass: "directory", roles: ["AgentControl.Operator"], capabilityId: "graph.directory.read" }, async (request, response) => {
...
policyRoute(agentsRouter, "post", "/directory/principals/resolve", { access: "authenticated", dataClass: "directory", roles: ["AgentControl.Operator"], capabilityId: "graph.directory.read", csrf: true }, async (request, response) => {
...
policyRoute(agentsRouter, "post", "/agents/refresh-jobs", { access: "authenticated", dataClass: "private_inventory_job", roles: ["AgentControl.Reader"], csrf: true }, async (request, response) => {
...
policyRoute(agentsRouter, "post", "/agents/:id/refresh-jobs", { access: "authenticated", dataClass: "private_inventory_job", roles: ["AgentControl.Reader", "AgentControl.Operator"], csrf: true }, async (request, response) => {
  const tokenMode = packageMode(request.body?.mode);
...
policyRoute(agentsRouter, "get", "/agents/refresh-jobs", { access: "authenticated", dataClass: "private_inventory_job", roles: ["AgentControl.Reader", "AgentControl.Operator"] }, async (request, response) => {
...
policyRoute(agentsRouter, "get", "/agents/refresh-jobs/:id", { access: "authenticated", dataClass: "private_inventory_job", roles: ["AgentControl.Reader", "AgentControl.Operator"] }, async (request, response) => {
...
policyRoute(agentsRouter, "post", "/agents/refresh-jobs/:id/resume", { access: "authenticated", dataClass: "private_inventory_job", roles: ["AgentControl.Reader", "AgentControl.Operator"], csrf: true }, async (request, response) => {
  response.status(202).json(await startPackageRefreshOrWaiting(request, jobId(request), packageMode(request.body?.mode)));
});
policyRoute(agentsRouter, "get", "/agents/snapshots", { access: "authenticated", dataClass: "private_inventory", roles: ["AgentControl.Reader"] }, async (request, response) => {
...
policyRoute(agentsRouter, "get", "/agents/export.csv", { access: "authenticated", dataClass: "private_inventory_export", roles: ["AgentControl.Reader"] }, async (request, response) => {
  await sendPackageExport(request, response, { ...packageListQuery(request.query), limit: 5_000, offset: 0 });
});
policyRoute(agentsRouter, "post", "/agents/export.csv", { access: "authenticated", dataClass: "private_inventory_export", roles: ["AgentControl.Reader"], csrf: true }, async (request, response) => {
...
policyRoute(agentsRouter, "post", "/agents/details", { access: "authenticated", dataClass: "private_inventory", roles: ["AgentControl.Reader"], csrf: true }, async (request, response) => {
  ...
  const results = await Promise.all(ids.map(async id => {
    ...
    return observed?.package ? { id, status: "succeeded" as const, package: inventoryPackageDetail(observed.package) } : { id, status: "failed" as const, message: "The saved package target is absent or stale; run an explicit exact refresh." };
  }));
  response.json({
...
policyRoute(agentsRouter, "get", "/agents/bulk-jobs/:id", { access: "authenticated", dataClass: "private_job", roles: ["AgentControl.Operator"] }, async (request, response) => {
...
policyRoute(agentsRouter, "get", "/agents/bulk-jobs", { access: "authenticated", dataClass: "private_job", roles: ["AgentControl.Operator"] }, async (request, response) => {
  const scope = requestScope(request);
  await bulkJobs.recover(scope.tenantId);
  response.json(await bulkJobs.list(scope, positiveInteger(firstQueryValue(request.query.limit), 20, 50)));
});
policyRoute(agentsRouter, "post", "/agents/bulk-jobs/:id/cancel", { access: "authenticated", dataClass: "private_job", roles: ["AgentControl.Operator"], csrf: true }, async (request, response) => {
  const job = await bulkJobs.cancel(jobId(request), requestScope(request));
  if (!job) throw new AppError(404,"not_found","Job was not found.");
  response.json(job);
});
policyRoute(agentsRouter, "post", "/agents/bulk-jobs/:id/resume", { access: "authenticated", dataClass: "private_job", roles: ["AgentControl.Operator"], csrf: true }, async (request, response) => {
...
policyRoute(agentsRouter, "post", "/agents/bulk-jobs/:id/reconcile", { access: "authenticated", dataClass: "private_job", roles: ["AgentControl.Operator"], csrf: true }, async (request, response) => {
  response.json(await reconcileBulkJob(jobId(request), requestScope(request)));
});
policyRoute(agentsRouter, "get", "/agents/:id", { access: "authenticated", dataClass: "private_inventory", roles: ["AgentControl.Reader", "AgentControl.Operator"] }, async (request, response) => {
  const id = exactId(String(request.params.id));
  ...
  response.json({
    ...(request.session.user!.roles.includes("AgentControl.Operator") ? observed.package : inventoryPackageDetail(observed.package)),
...
policyRoute(agentsRouter, "post", "/agents/mutation-preview", { access: "authenticated", dataClass: "private_inventory", roles: ["AgentControl.Operator"], csrf: true }, async (request, response) => {
  const action = packageMutationAction(request.body?.action);
  requirePackageMutationOperationSafe(action);
...
policyRoute(agentsRouter, "post", "/agents/mutation-canaries", { access: "authenticated", dataClass: "package_control_qualification", roles: ["AgentControl.Administrator"], csrf: true }, async (request, response) => {
  const approval = parseCanaryApproval(request.body);
  requirePackageMutationOperationSafe(approval.action);
...
policyRoute(agentsRouter, "post", "/agents/mutation-canaries/:id/execute", { access: "authenticated", dataClass: "package_control_qualification", roles: ["AgentControl.Operator"], csrf: true }, async (request, response) => {
  ...
  requirePackageMutationOperationSafe(originalApproved.action);
  requirePackageMutationOperationSafe(restorationApproved.action);
  const owner = requestScope(request);
  const claimValidation = beginAccountSessionValidation(owner.tenantId, owner.principalId);
  const operator = await revalidateCanaryOperator(owner);
  ...
  try {
    ...
    const currentOperator = await revalidateCanaryOperator(owner);
    const currentOriginalIdentity = await capabilities.packageQualificationIdentity(claimed.original.action, currentOperator);
    const currentRestorationIdentity = await capabilities.packageQualificationIdentity(claimed.restoration.action, currentOperator);
    ...
    await commitAccountSessionValidation(publicationValidation, async () => {
      completed = await mutationQualifications.completeCycle(currentOperator, claimed.original.id, claimed.restoration.id, { status: "qualified" }, currentOriginalIdentity, currentRestorationIdentity);
    });
    response.json({
      status: "qualified",
...
for (const action of ["block","unblock"] as const) {
  policyRoute(agentsRouter, "post", `/agents/${action}-all`, { access: "authenticated", dataClass: "package_control", roles: ["AgentControl.Operator"], capabilityId: "graph.package.block.manage", csrf: true }, async (request, response) => {
    ...
    if (!request.session.user!.roles.includes("AgentControl.Reader")) throw new AppError(403, "missing_internal_role", "Block-all requires Reader catalog scope in addition to Operator control authority.");
  ...
  policyRoute(agentsRouter, "post", `/agents/${action}`, { access: "authenticated", dataClass: "package_control", roles: ["AgentControl.Operator"], capabilityId: "graph.package.block.manage", csrf: true }, async (request, response) => {
  ...
  policyRoute(agentsRouter, "post", `/agents/:id/${action}`, { access: "authenticated", dataClass: "package_control", roles: ["AgentControl.Operator"], capabilityId: "graph.package.block.manage", csrf: true }, async (request, response) => {
...
policyRoute(agentsRouter, "post", "/agents/access", { access: "authenticated", dataClass: "package_control", roles: ["AgentControl.Operator"], capabilityId: "graph.package.access.manage", csrf: true }, async (request, response) => {
...
policyRoute(agentsRouter, "patch", "/agents/:id/access", { access: "authenticated", dataClass: "package_control", roles: ["AgentControl.Operator"], capabilityId: "graph.package.access.manage", csrf: true }, async (request, response) => {
  const update = parsePackageAccessUpdate(request.body);
...
function authorizePackageFilters(request: Request, query: PackageListQuery): PackageListQuery {
  ...
  if (!request.session.user?.roles.includes("AgentControl.SecurityReader")) {
    throw new AppError(403, "missing_internal_role", "SecurityReader is required for audit operation reference filters.");
  }
...
function canaryJobAuthorizer(
  ...
  return async (scope: ReturnType<typeof requestScope>, capabilityId: Parameters<typeof acquireDelegatedToken>[1]) => {
    ...
    const current = await revalidateCanaryOperator(scope);
...
async function revalidateCanaryOperator(scope: ReturnType<typeof requestScope>) {
  const user = await revalidateAuthenticatedUser(scope.principalId);
  if (user.tenantId !== scope.tenantId || user.homeAccountId !== scope.principalId) throw AppError.unauthorized("The canary Operator no longer matches the signed-in account.");
  if (!user.roles.includes("AgentControl.Operator")) throw new AppError(403, "missing_internal_role", "AgentControl.Operator is required for the full canary cycle.");
  return user;
}
...
async function sendPackageExport(request: Request, response: Response, query: PackageListQuery) {
  ...
  const validateAuditSession = query.operationIdPrefix ? createExportPublicationValidator(request, "AgentControl.SecurityReader") : undefined;
  const mode = packageMode(firstQueryValue(request.query.mode));
