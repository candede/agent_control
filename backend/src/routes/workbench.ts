...
policyRoute(workbenchRouter, "get", "/workbench/jobs", {
  ...
  if (user.roles.includes("AgentControl.Reader") || user.roles.includes("AgentControl.Operator")) {
    loaders.push({ source: "package-refresh", load: async () => (await packageRepository.listJobs(scope, user.homeAccountId, 20)).value.map(job => ({
      ...
      canResume: job.status === "waiting_authorization", canCancel: false, canReconcile: false,
      updatedAt: job.updatedAt,
      href: `/agents?refreshJob=${encodeURIComponent(job.id)}${job.tokenMode === "application" ? "&mode=application" : ""}` as const,
    })) });
  }
  if (user.roles.includes("AgentControl.Reader")) {
  ...
  if (user.roles.includes("AgentControl.Operator")) {
    loaders.push({ source: "package-controls", load: async () => (await bulkJobs.list(scope, 20)).value.map(job => ({
      ...
      canResume: job.canResume, canCancel: ["queued", "running", "waiting_authorization"].includes(job.status),
      canReconcile: job.results.some(result => result.status === "inconclusive" && result.reconciliationStatus === "required"),
  ...
  if (user.roles.includes("AgentControl.Administrator")) {
